...
@Injectable()
export class Ng2Fone {
    gotdevice:any = {};
    
    //Got this list from Jared Rummler and changed it into json. All credits go to him.
        deviceinfo:any = [
            {
                "brandname":"Acer",
    ...
    //this method gets the market name of the specified modelno
    
    getmobilename(modelno: string) {
         for (var i = 0; i < this.deviceinfo.length; i++) {
                    if (modelno == this.deviceinfo[i].Model) {
                        this.gotdevice = {
                            brandname: this.deviceinfo[i].brandname,
                            marketname: this.deviceinfo[i].marketname,
                            labelname: this.deviceinfo[i].labelname,
                            model: this.deviceinfo[i].Model
                        }
                        return this.gotdevice;
                    }
    }
}
    }